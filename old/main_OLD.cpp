...
void trace(const struct v_f ray_direction,
  ...
  if (object < f_count) {
    ...
    unsigned int idx = f_mem[object * 4 + 3];  // direct array access, no pgm_read_word

    // Ambient
    if (mtl[idx][0]) {
      diffuse_value -= mtl[idx][0];
      double ambient_pct = mtl[idx][0] / 255.0;
      color[0] = (mtl[idx][3] * ambient_pct);
      color[1] = (mtl[idx][4] * ambient_pct);
      color[2] = (mtl[idx][5] * ambient_pct);
    }
    ...
    if (mtl[idx][1]) {
      ...
      color[0] += reflect_clr[0] * reflect_pct;
      color[1] += reflect_clr[1] * reflect_pct;
      color[2] += reflect_clr[2] * reflect_pct;
    }
    ...
    if (mtl[idx][2]) {
      ...
      color[0] += (transparent_clr[0] * transparent_pct);
      color[1] += (transparent_clr[1] * transparent_pct);
      color[2] += (transparent_clr[2] * transparent_pct);
    }

    // Diffuse
    if (diffuse_value) {
      double diffuse_pct = (double)(diffuse_value) / 255.0;
      ...
      if (lit) {
        ...
        color[0] += (mtl[idx][3] * shade);
        color[1] += (mtl[idx][4] * shade);
        color[2] += (mtl[idx][5] * shade);
      }
    }
...
int main() {
  ...
  // Ray setup
  double lerp_rate = 2.0 * tan(half_fov) / imageWidth;
  double half_width = (imageWidth - 1) / 2.0;

  struct v_f ray_origin;
  v_f_create_f_f_f(ray_origin, 0, 0, 0);

  // Framebuffer for thread-safe parallel rendering
  const size_t pixel_count = static_cast<size_t>(imageWidth) * static_cast<size_t>(imageWidth);
  ...
  for (int y = 0; y < (int)imageWidth; y++) {
    double v = y - half_width;
    for (unsigned int x = 0; x < imageWidth; x++) {
      double u = x - half_width;

      struct v_f ray;
      v_f_create_f_f_f(ray, u * lerp_rate, v * lerp_rate, -1.0);
      v_f_norm(ray);

      unsigned int color[3] = {0, 0, 0};
      trace(ray, ray_origin, 0, color);

      size_t idx = (static_cast<size_t>(y) * imageWidth + x) * 3;
    ...
    // Progress every 5s. Only one thread prints, because chaos is fun but not that fun.
    #pragma omp critical
    {
      ...
      if (elapsed - last_print >= 5) {
        last_print = elapsed;
        unsigned int pct = (y * 100) / imageWidth;
        printf("  %3u%% | row %d/%d | %lds elapsed\n",
               pct, y, imageWidth, elapsed);
        fflush(stdout);
      }
    }
  }
